...
dist(nyc, method="manhattan", diag=F, upper=TRUE);  #a literal distance matrix with "taxi cab" distances between all of these locations in manhattan in coordinate units??
...
dist(nyc[,3:4], method="manhattan", diag=FALSE, upper=TRUE); #manhattan distance in meters

#this works, times square to the bull on wall street is listed as 4 miles on google maps
...
The angle of rotation will influence the coordinates that define the location, but should not change the Manhattan Distance. If the coordinate vectors are the same length, than ultimately those distances should be the same.
...
Di Fara Pizza is across the water, therefore the exact distance isn't practical information for actually travelling there. The distance needs to incorporate access to a bridge, then distance from the bridge to Di Fara Pizza.  

####  *Question 3:*  Which distance metric seems to be most conservative (overstating distance:  Manhattan, Euclidean, Haversine)? 
Manhattan distance appears to be most conservative, followed by Haversine, followed by Euclidean.  Although when the shape of the Earth is considered, it is possible that Manhattan distance is too 2-dimensional and will end up undershooting the distance as a result.

####  *Question 4:*  Which Spherical Ellipsoid Distance is most accurate (Haversine, Meeus, or Geo)?  How can you verify that?
Haversine produces larger distances relative to Meeus, which produces larger distances relative to Geo based on the Chicago distance set.  According to some R documentation, Geo is the most accurate, although it isn't clear entirely why, but perhaps that the ellipsoid that is used in the calculations most closely represents the globe.  You could verify that in silico by applying Haversine or Meeus distances to the best globe approximation.  You could verify that in situ by actual travel? assuming that external variables such as wind are accounted for.
...
##my code solution to evaluating the ellipsoid distance with the distGeo function, then determining whether the provided locations relative to the zipcode really fall within the search query
...
```{r}
## I repeated the exercise for a zip code in Salt Lake City, with a wider mile radius
...
## finally, filter results that are within the specified radius per the incircle results
...
## my question - is this the concept behind fuzzy search? some kind of "tolerance" for strings?
...
#What is the term for this type of equality (e.g., `ant` and `tan` mixing up the same characters to form a new string that has its own meaning)? 
Anagram?  
#Is there a string function that would find such examples?
We can use distances to identify identical compositions of letters.  In theory, you could also write a function that examines each string for its character components, and determine how they compare, although that would be extremely inefficient.

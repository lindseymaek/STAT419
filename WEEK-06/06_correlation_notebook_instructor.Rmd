...
T-tests are used to test the null hypothesis of equal means for two small samples (of equal size?) that are normally distributed. If two sided, the null hypothesis is that the means of the two groups are equal, and the alternative hypothesis is that they are not, In a one-sided t-test, the null hypothesis is that

```{r, chunck-student-t}
t.test(x.0.2, x.0.3, alternative = "greater", conf.level = 0.05, paired=FALSE, var.equal=FALSE);
...
plot(x.0.1, x.0.1b);
    ...
    cor(x.0.1, x.0.1b, method="spearman"); # this is non-parametric
...
##this is useful because the correlation values as well as the pvalues are provided
...
##what does predict linear model do?
...
#wingspan vs arm reach #significant pval, strong positive correlation
cor.test(myMeasureData$arm.span...NA, myMeasureData$arm.reach...right, method = "pearson")

#foot length vs head height #pval not significant, weak negative correlation
cor.test(myMeasureData$foot.length...left, myMeasureData$head.height...NA, method = "pearson")

#hand span vs hand height
cor.test(myMeasureData$hand.length...right, myMeasureData$hand.width...right, method = "pearson")
...
## this gives me an NA - what is that? #solve with use complete.obs
Y = (((X^(0.5)*sd(X)+X^2)^.123 / (mean(X) + (X - 10*X)- X^2)^11) )+(2/X);

...
Me: Or in Wine! where you are trying to understand how treatments may cluster
...
##suitability tests
#kmo test
Xs.self.KMO = KMOS(Xs.self);
Xs.self.KMO$KMO # 0.903 > 0.90, this is encouraging 
#bartlett sphericity test
Xs.self.corr = cor(Xs.self);
Xs.self.bartlett = cortest.bartlett(Xs.self.corr, n = nrow(Xs.self));
Xs.self.bartlett$p.value # is 0 < 0.05, good reason to believe this data is suitable for MFA

##factors needed
Xs.self.corr.eigen = eigen(Xs.self.corr); 

Xs.self.corr.eigen$values[Xs.self.corr.eigen$values > 1];# suggests 4-5, 6 appoaches 1

self.nResults = nScree(eig = Xs.self.corr.eigen$values,
              aparallel = parallel(
                              subject = nrow(Xs.self), 
                              var = ncol(Xs.self) )$eigen$qevpea);

plotnScree(self.nResults, main="Component Retention Analysis");  #parallel and optimal coordinates suggest 4

##assuming 4 factors, a summary analysis

round( psych::describe(Xs.self), digits=5);

Xs.factanal.4 = factanal(Xs.self, factors=4, rotation='none');


# Uniqueness
head(Xs.factanal.4$uniquenesses);
# "Uniqueness" shows what???


# Map of Variables to Factors (Loadings)
print(Xs.factanal.4$loadings, digits=2, cutoff=0.25, sort=FALSE);

plot(Xs.factanal.4$loadings[,1:2], type="n");
text(Xs.factanal.4$loadings[,1:2],labels=names(Xs),cex=.7) # add variable names
```

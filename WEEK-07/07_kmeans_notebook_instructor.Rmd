...
I think I was previously thinking about scaling incorrectly - where I would scale datasets individually, then combine them in multifactor analysis.  I understand how this is problematic, because scaling and centering relies on the relation of the data to itself, which will change based on that data.
...
```
3-4 is a reasonable amount to cluster - others can be identified, but then require justification as to the variability that is being explained. 

Question - I understand that these stars plots are characterizing the clusters we created by kmeans - but how is this variability being defined? How is cluster 2 being associated with cereals and nuts...is this simply a mattter of count?

Monte's answer - the mathematics of this have been left out because they aren't immediately necessary to the concept, but should be explored to address fundamental questions like this.  

The PCA is the engine behind any factors analysis
...
```
Hmmm, based on the au pvalues, I assumed that these clustered really well - that is an apples-to-oranges comparison.  With hierarchical clustering and kmeans vs pca we are talking about the difference between looking at pairwise comparisons and considering all of the data in total.  
...
K = 2:4;  # let's try these choices for our "k"
n.K = length(K);
...
```
I get an error in this code,that says plot not found - I'm not certain what is happen, but in general the point is communicated. Membership to clusters is definitely method dependent - I would imagine you can either try and justify this with multiple methods, or by selecting one that is preferrred for the field.
...
# My justificatiton 2 is too small - a lot of variability is hidden within the second starplot and I think that 3 separates this out nicely. When you add a 4th star plot, you get some over lap with 2 and 3 that, while perhaps expalining some nuanced variability, no longer nicely clusters participation by protein
...
Hand et al. 1994 pp 297. A study that quantified the protein consumption by food group in 25 different countries in the 1970s. I am not actually totally clear on how these numbers were calculated - average grams per person per day?  If I were to do an actual research assessment of this, that would be important to know, but what matters for this is that the data are in comparable units.

## TASK: Research Question

Ex; I would like to identify which countries have similar protein-consumption habits. ...

--WRITE SOMETHING HERE--
I would like to identify whether country participation in a protein cluster is related to the proximity of that country to other countries in the cluster.

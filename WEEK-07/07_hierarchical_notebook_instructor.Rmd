...
#protein.file = paste0(local.path,"protein.txt");
...
# why don't we scale the data first?
#...why don't we scale the data first? it's interesting to see just pick a height, and see how the number of clusters varies with each method
#UK, Belgium, France and Ukraine, Yugoslavia, and Romania are consistently close
...
## trying the scaling exercise from below
Xs = scale(X);
...
...what are au p-values? 

How does this work - it does seem like the lower the number, the greater the distance...
Found this source https://www.datanovia.com/en/lessons/computing-p-value-for-hierarchical-clustering/#description-of-pvclust-function
Which indicates the following about pvalues:
au p-values = approximately unbiased pvalue - it's a way of defining probability associated with the clustering. Clusters with an AU of greater than 95% are supported by the data, although perhaps this acceptance criteria varies depending ong the question and confidence needed

the bp or bootstrap probabilities = frequency the cluster is observed in bootstrap copies. No acceptance criteria is stated for this?

This is interesting, based on the pvalues, it appears we can have confidence in the clusters.
...
--WRITE SOMETHING HERE--
The over all "scale" is smaller - or the height that is listed on the y-axis. Just from past experience with PCA, I understand that scaling can be important for controlling for over approximations of distance that may result from large differences, although I don't totally understand in principle what that means...

Many of the country clusters that I identified previously are the same.

It makes sense to scale the data...
...
```{r}
##re-define X in order to look at protein distances instead with hclust
X = protein.t
Xs = scale(X);
...
## swapping, protein for protein.t in pvclust
X = removeColumnsFromDataFrame(protein,"Country");
rownames(X) = protein$Country;
Xs = scale(X);
...
for(method in methods)
  {
  time.start = Sys.time();  
      X.pvclust = pvclust ( Xs, method.hclust=method);
  plot(X.pvclust);
      pvrect(X.pvclust);
  time.end = Sys.time();
  elapse = sprintf("%.3f", as.numeric(time.end) - as.numeric(time.start));
...
--WRITE SOMETHING HERE--
Body parts - in the full dataset, there will be a high volume of participants that will make it difficult to interpret those differences.
...
myMeasureData <- read.csv(file = paste0(local.path, "datasets/measure/measure.txt"), sep = "|", header = T)
d = myMeasureData[,-c(1:2, 26:36)]

#cols = colnames(protein);
rows = myMeasureData$person_id;
d.t = transposeMatrix(d);
colnames(d.t) = rows;
...
##interesting, are the groupings just a function of the relative lengths? I suppose that makes sense?

...
I think that 6 cuts is potentially too many, because there will not be enough distinguishable features in each of the numbers. I do like the idea in the video that suggests using proportions of data present in a given sector as a potential criteria, although I think this would require some fine tuning of the tolerance, or particular centeing of each number. 
...
-- The zero has several features that the two doesn't have - curvature on the bottom left side, and data that is present on the left side of the character, where the 2 is blank. A criteria could be put in place such that the data in the left middle section must be continuous. --
...
if I replace 3 with 6, that does improves
```{r}

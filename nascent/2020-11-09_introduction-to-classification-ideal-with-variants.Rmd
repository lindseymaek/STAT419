...
MNIST stands for Modified National Institute of standards and technology. The MNIST dataset is a collection of thousands of images that contain 28x28 pixel handwritten, grayscale numbers, represented as a matrix of 748 comma-separated pixel values


The distance between two images is the collection of euclidean distances between corresponding pixel values of each image.
...
```{r}
source( paste0(path.github,"humanVerseWSU/R/functions-EDA.R") );
...
The row sum was the best predictor for the correct number, followed by the median, then the top value, then the mean. I think the row sum is powerful in this case because it represent many numbers, rather than just a single value, or a calculated value.
